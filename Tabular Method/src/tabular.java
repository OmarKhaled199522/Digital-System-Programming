...
public class tabular {
					...
					for(int j = i + 1; j < l; j++){
						...
						if(  ((Single_Linkedlist) arr[i]).size() == counter && ((Single_Linkedlist) arr[j]).size() == counter){
							...
							if(  check_for_big_pair( (Single_Linkedlist)arr[i]  ,  (Single_Linkedlist)   arr[j])){
								if( check_of_vaild0( (int) ((Single_Linkedlist) arr[j]).get(0), (int) ((Single_Linkedlist) arr[i]).get(0))  ){
									vis_first[index_of_implicant++] = vis_first[index_of_implicant++] = true;
									cancel(  (Single_Linkedlist) arr[i] );
									cancel( (Single_Linkedlist) arr[j] );
									is_there_pair = true;	
									int tempo =  (int) ((Single_Linkedlist) arr[j]).get(0) - (int) ((Single_Linkedlist) arr[i]).get(0);
									arr[l] = new Single_Linkedlist();
									//arr[l] = arr[i];
	...
	public boolean equality (Single_Linkedlist l1 , Single_Linkedlist l2){
		...
		memset();
		if((int)l1.get(0) != (int)l2.get(0)) return false;
		for(int i = 1; i < l1.size(); i++){
			compare1[ (int) l1.get(i)] = 1;
			compare2[ (int) l2.get(i)] = 1;
		}
	...
	public void prime_implicant(){
		...
		for(int i = 0; i < l; i++){
			...
			int summ = 0;
			while(co != null){
				if (not_care[ summ + (int) co.value] != 1){
					enter = 1;
				}
				summ +=  (int) co.value;
				co = co.next;
	...
	public void set_table(){
		...
		for(int i = 0; i < s ; i++){
			...
			while(f != null){
				...
				if( (f == g.getHead() && not_care [ (int) f.value ] == 1)   || (f != g.getHead() && not_care[ (int) f.value + (int) org.value ] == 1)) {f = f.next;continue;}
				
				
				if(f != g.getHead())   table[i][(int) f.value + (int) org.value] = true;
				else  table [i][ (int) org.value] = true;
				sum += (int) f.value;
				num++;
				if (f != g.getHead())  x_num_col [(int) f.value + (int) org.value]++;
				else				   x_num_col [(int) org.value]++;
				f = f.next;
			}
			if(num > 2) num++;
			cont_x += num;
	...
	public void final_print(){
		...
		for(int i = 0; i < size_of_eesential; i++){
			...
			String s1 = Integer.toBinaryString( (int) l.getHead().value );
			/*while(s1.length() < siz){
				s1 += '0';
			}*/
